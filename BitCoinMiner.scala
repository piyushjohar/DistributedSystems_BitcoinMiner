...
object BitCoinMiner {
	...
	class Listener extends Actor {
		...
		def receive = {
			case StopAndPrintExecutionSummary(size, duration) => {
				println("\n\tNumber of Minned Strings : \t\t%s\n\tCalculation time: \t%s".format(size, duration))
	...
	var nrOfWorkers = 10 // no of Worker Actors to create on invoking the program.
	var zeroCountReq = 2
	var ipAddress = ""
	var nrOfStrings = 10000000
